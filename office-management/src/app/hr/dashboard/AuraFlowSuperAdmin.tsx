...
export default function AuraFlowSuperAdmin({ initialDepts = [], existingEmployees = [], initialProjects = [], hrUser }: any) {
  ...
  return (
    <div className="h-screen w-full bg-[#050505] text-white font-sans flex flex-col overflow-hidden selection:bg-[#FFD541] selection:text-black">
        ...
        <div className="max-w-[1750px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-12">
            ...
            <div className="flex gap-8">
              {['Dashboard', 'Organization', 'Workforce', 'Payroll'].map(t => (
        ...
        <div className="max-w-[1750px] mx-auto space-y-8">
          ...
          <header className="px-2 text-left">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest text-white/20">Executive Terminal • {hrUser?.organization || "AuraFlow Corp"}</p>
            </div>
            <h1 className="text-4xl font-bold tracking-tight text-white uppercase">Operational Intelligence</h1>
          </header>
          ...
          <AnimatePresence mode="wait">
            ...
            {/* TAB: PAYROLL */}
            {activeTab === "Payroll" && (
              <motion.div key="pay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white/[0.03] backdrop-blur-3xl rounded-[3rem] border border-white/10 overflow-hidden text-left">
                <div className="p-8 border-b border-white/5 flex justify-between items-center">
                  <h3 className="text-xl font-bold uppercase tracking-widest text-white">Workforce Payout Ledger (Rs.)</h3>
                  <div className="flex bg-white/5 p-1 rounded-xl border border-white/5">
                      {['HOUR', 'DAY', 'MONTH'].map(m => (
                        <button key={m} onClick={() => setSalaryMode(m as any)} className={`px-5 py-2 rounded-lg text-[10px] font-bold tracking-widest transition-all ${salaryMode === m ? 'bg-[#FFD541] text-black' : 'text-white/20 hover:text-white'}`}>{m}</button>
                      ))}
                  </div>
                </div>
                  ...
                  <table className="w-full text-left">
                    <thead>
                      <tr className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em] border-b border-white/5">
                        <th className="px-6 py-4">Personnel Identity</th><th className="px-6 py-4 text-center">Gross Reference</th><th className="px-6 py-4 text-right">Audit Action</th>
                      </tr>
                    </thead>
                    <tbody className="text-[13px] font-medium text-white/70">
                      {employees.map((emp, i) => (
                        <tr key={i} className="border-t border-white/5 hover:bg-white/[0.02] transition-colors group">
                          <td className="px-6 py-5 text-white font-bold">{emp.name}</td>
                          <td className="px-6 py-5 text-center font-bold text-[#FFD541]">
                            <button onClick={() => {
                                setFiscalTarget(emp);
                                setContractBaseline(emp.salary?.toString() || "85000");
                               ...
                               <div className="flex items-center gap-2"><FileSignature size={12} /> Audit Contract</div>
                             </button>
                          </td>
                          <td className="px-6 py-5 text-right flex justify-end gap-3">
                             <button
                               className="px-4 py-2 bg-white/5 text-white/40 rounded-lg text-[10px] font-bold uppercase tracking-widest hover:bg-white/10 hover:text-white transition-all shadow-lg"
                               onClick={() => {
                                 setSelectedAuditEmployee(emp);
                                 setShowAuditModal(true);
                               }}
                             ><div className="flex items-center gap-2"><History size={14} /> historical ledger</div></button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {activeTab === "Organization" && (
                <motion.div key="org" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 text-left">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-5 gap-6">
                        {departments.map((dept: any) => (
                            <div key={dept.id} onClick={() => { setSelectedDept(dept); setIsDetailModalOpen(true); }} className="bg-white/5 backdrop-blur-2xl rounded-3xl p-6 border border-white/10 hover:border-[#FFD541]/40 cursor-pointer transition-all group">
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-white/5">
                                    <span className="text-[10px] font-bold text-white/40">{dept.members} Personnel</span>
      ...
      {/* --- CUSTOM FISCAL TRIGGER MODAL (REPLACING image_a5c41d.png) --- */}
      <AnimatePresence>
        {isFiscalTriggerOpen && (
          <div className="fixed inset-0 z-[700] flex items-center justify-center p-6 text-left">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsFiscalTriggerOpen(false)} className="absolute inset-0 bg-black/80 backdrop-blur-xl" />
            <motion.div initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.98, opacity: 0 }} className="relative bg-[#0A0A0B] w-full max-w-xl rounded-[2.5rem] p-10 border border-white/10 shadow-[0_50px_100px_rgba(0,0,0,0.8)] overflow-hidden">
               <div className="absolute top-0 left-0 w-full h-1.5 bg-[#FFD541] opacity-40" />
               <div className="flex items-center gap-4 mb-8">
                  <div className="w-12 h-12 bg-[#FFD541]/10 rounded-2xl flex items-center justify-center text-[#FFD541]"><Banknote size={24} /></div>
                  <div><h2 className="text-xl font-bold text-white uppercase tracking-tight">Contract Audit</h2><p className="text-[9px] font-bold text-white/30 uppercase tracking-[0.2em]">{fiscalTarget?.name}</p></div>
               </div>

               <div className="space-y-5">
                  <div className="space-y-1.5">
                     <label className="text-[8px] font-black uppercase tracking-[0.2em] text-white/30 ml-1">Contractual Baseline (Rs.)</label>
                     <input value={contractBaseline} onChange={e => setContractBaseline(e.target.value)} className="w-full bg-white/5 border border-white/5 focus:border-[#FFD541]/40 rounded-xl py-3.5 px-6 text-sm text-white outline-none transition-all font-bold" />
                  </div>
                  <div className="space-y-1.5">
                     <label className="text-[8px] font-black uppercase tracking-[0.2em] text-white/30 ml-1">Operational Deduction (Rs.)</label>
                     <input value={performanceDeduction} onChange={e => setPerformanceDeduction(e.target.value)} className="w-full bg-red-500/5 border border-red-500/10 focus:border-red-500/40 rounded-xl py-3.5 px-6 text-sm text-red-400 outline-none transition-all font-bold" />
                  </div>
                  <div className="space-y-1.5">
                     <label className="text-[8px] font-black uppercase tracking-[0.2em] text-white/30 ml-1">Authorization Note</label>
                     <textarea value={authNote} onChange={e => setAuthNote(e.target.value)} className="w-full bg-white/5 border border-white/5 focus:border-[#FFD541]/40 rounded-2xl p-5 text-[11px] text-white/60 outline-none transition-all resize-none h-24" />
                  </div>
               </div>

               <div className="flex gap-3 mt-8">
                  <button onClick={() => setIsFiscalTriggerOpen(false)} className="flex-1 py-4 bg-white/5 text-white/40 rounded-full font-black uppercase text-[10px] tracking-widest hover:bg-white/10 transition-all">Abort</button>
                  <button onClick={handleFiscalAudit} className="flex-[2] py-4 bg-[#FFD541] text-black rounded-full font-black uppercase text-[10px] tracking-widest shadow-xl shadow-yellow-500/10 active:scale-95 transition-all"><div className="flex items-center justify-center gap-2"><Send size={14} /> Transmit</div></button>
               </div>
            </motion.div>
          </div>
        )}
